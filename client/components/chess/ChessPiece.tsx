...
// Enhanced SVG XML strings for each piece with improved contrast and clarity
// White pieces
const whitePawnSvg = `
...
<svg xmlns="http://www.w3.org/2000/svg" width="45" height="45">
  <g fill="#000" fill-rule="evenodd" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <path d="M12 35.5h21m-20-4h19m-18-2h17m-17-13h17M11 14h23" stroke="#fff" stroke-linejoin="miter"/>
  </g>
</svg>`;
...
<svg xmlns="http://www.w3.org/2000/svg" width="45" height="45">
  <g fill="#000" fill-rule="evenodd" stroke="#000" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
    ...
    <path d="M9 26c0 2 1.5 2 2.5 4 1 1.5 1 1 .5 3.5-1.5 1-1.5 2.5-1.5 2.5-1.5 1.5.5 2.5.5 2.5 6.5 1 16.5 1 23 0 0 0 1.5-1 0-2.5 0 0 .5-1.5-1-2.5-.5-2.5-.5-2 .5-3.5 1-2 2.5-2 2.5-4-8.5-1.5-18.5-1.5-27 0z" stroke-linecap="butt"/>
    <path d="M11 38.5a35 35 1 0 0 23 0" stroke-linecap="butt"/>
    <path d="M11 29a35 35 1 0 1 23 0m-21.5 2.5h20m-21 3a35 35 1 0 0 22 0m-23 3a35 35 1 0 0 24 0" stroke="#fff"/>
  </g>
</svg>`;
...
// Map piece types to SVG XML strings
const svgMap = {
  p: {
    w: whitePawnSvg,
    b: blackPawnSvg,
...
const ChessPiece: React.FC<ChessPieceProps> = ({ type, color, size }) => {
  ...
  const svgXml = svgMap[type][color];
  
  // Enhanced styling for better visibility in dark mode
  const shadowStyle = {
    shadowColor: isDark ? '#FFFFFF' : '#000000',
    shadowOffset: { width: 0, height: color === 'b' && isDark ? 1 : 2 },
    shadowOpacity: color === 'b' && isDark ? 0.7 : 0.4,
    shadowRadius: color === 'b' && isDark ? 4 : 3,
    elevation: 5,
  };
  
  // Add slight scaling for better visibility
  const scale = color === 'b' && isDark ? 0.85 : 0.8;
  
  return (
    <View style={[styles.pieceContainer, { width: size, height: size }]}>
      <View style={[styles.pieceWrapper, shadowStyle]}>
        <SvgXml 
          xml={svgXml}
          width={size * scale}
          height={size * scale}
...
const styles = StyleSheet.create({
  ...
  pieceWrapper: {
    // Shadow styles moved to dynamic styling above
  }
});

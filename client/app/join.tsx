...
import { View, Text, TextInput, StyleSheet, TouchableOpacity, ActivityIndicator, Alert, SafeAreaView, ColorValue } from 'react-native';
...
const JoinGameScreen = () => {
  ...
  const handleJoinGame = async () => {
    ...
    try {
      // Make API request to join the game
      const response = await fetch(`${getServerUrl()}/api/join-game`, {
        method: 'POST',
        headers: {
      ...
      Alert.alert('Error', 'Could not connect to the server. Please check that the server is running and accessible.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <View style={styles.overlay}>
          ...
          <View style={styles.content}>
            <BlurView intensity={isDark ? 30 : 60} tint={isDark ? "dark" : "light"} style={styles.formContainer}>
              ...
              >
                <Text style={[styles.label, { color: isDark ? Colors.dark.text : Colors.light.text }]}>Game ID</Text>
                <TextInput
                  style={[
                    styles.input, 
                    { 
                ...
                <TouchableOpacity
                  style={[
                    styles.button, 
                    (!gameId.trim() || !playerName.trim()) && styles.buttonDisabled
                  ]}
                ...
                >
                  <LinearGradient
                    colors={isDark 
                      ? ['#818CF8', '#6366F1'] as any
                      : ['#6366F1', '#4F46E5'] as any}
            ...
            <View style={styles.infoContainer}>
              <BlurView intensity={isDark ? 20 : 30} tint={isDark ? "dark" : "light"} style={[
                ...
                <Text style={[styles.infoTitle, { color: isDark ? Colors.dark.text : Colors.light.text }]}>How to join:</Text>
                <Text style={[styles.infoText, { color: isDark ? Colors.dark.textSecondary : Colors.light.textSecondary }]}>
                  1. Get the game ID from the person who created the game
                </Text>
                <Text style={[styles.infoText, { color: isDark ? Colors.dark.textSecondary : Colors.light.textSecondary }]}>
